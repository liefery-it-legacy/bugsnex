defmodule Bugsnex.PlugTest do
  ...
  test "build_plug_env/2" do
    ...
    plug_env = %{
      request: Bugsnex.Plug.build_request_data(conn),
      context: "/bang",
      params: %{
        "foo" => "bar",
        "password" => "[FILTERED]",
  ...
  test "get_hostname/0" do
    ...
    # depends on the system, so we just make sure it's a string
    assert is_binary(Bugsnex.Plug.get_hostname())
  end
